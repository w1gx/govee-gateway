...
public:
    ...
    const std::map<std::string, std::queue<std::string>>& SectionKeys() const;
...
protected:
    ...
    std::map<std::string, std::queue<std::string>> _sectionKeys;
...
inline const std::map<std::string, std::queue<std::string>>& INIReader::SectionKeys() const
{
    return _sectionKeys;
}
                            ...
                            const char* value)
{
    ...
    auto ret = reader->_sectionKeys.insert(std::pair<std::string, std::queue<std::string>>(section, q));
    ret.first->second.push(name);
    return 1;
}
